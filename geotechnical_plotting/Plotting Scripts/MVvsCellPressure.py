#!/usr/bin/env python3
"""
MV vs Cell Pressure - Simplified Dual-Parameter Plotting Script
================================================================
...
Plots Coefficient of Volume Compressibility (mv) against Cell Pressure - a
dual-parameter scatter plot rather than parameter vs depth.
...
# For MV vs Cell Pressure, Consolidation by Geology has both parameters
# ─────────────────────────────────────────────────────────────────────────
CSV_SOURCE_SETTINGS = {
...
CONFIG = build_dual_param_config_from_defaults(
    ...
    y_parameter_name="CoefficientVolumeCompressibility",
    x_display_name="Cell Pressure (kPa)",
    y_display_name="mv (m²/MN)",
    output_folder_name="CoefficientVolumeCompressibilityvsCellPressure",
    csv_source_settings=CSV_SOURCE_SETTINGS,
    # Optional: Manual data sheet name (uses y_parameter_name by default)
    manual_sheet_name="mv",
    ...
    filtering_override={
        ...
        # Formation-specific parameter thresholds (if needed)
        "formation_parameter_thresholds": {
            "enabled": False,  # Disabled by default - enable if formation-specific limits needed
            "thresholds": {},
        },
    },
    ...
    row_level_fallback_override={
        "enabled": True,
        "parameters": ["CoefficientVolumeCompressibility", "Cell Pressure"],
    },
    # Hide test type shapes in legend (only one test type for MV)
